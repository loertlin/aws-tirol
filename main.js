    ...
    L.geoJSON(geojson, {
        ...
        pointToLayer: function (geoJsonPoint, latlng) {
            ...
            return L.marker(latlng, {
                icon: L.divIcon({
                    className: "aws-div-icon",
                    html: `<span style="background-color:${color}; transform: rotate(${deg}deg)"><i class="fa-solid fa-circle-arrow-up"></i>${geoJsonPoint.properties.WG.toFixed(1)*3.6}km/h</span>`
                })
...
let drawHumidity = function (geojson) {
    ...
    L.geoJSON(geojson, {
        filter: function(geoJsonPoint) {
            if (geoJsonPoint.properties.RH > 0 && geoJsonPoint.properties.RH < 300) {
                return true;
            }
        },
        pointToLayer: function (geoJsonPoint, latlng) {
